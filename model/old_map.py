...
from utils import aligned_line, energy_color_map, energy_pip_symbol, numbered_list, choose_obj, choose_idx, choose_binary, colorize, choose_str
...
class Node:
  def __init__(self, safehouse, guardian_enemy_waves: List[EnemyWave], position, seen=False,
               difficulty=1, boss=False, page_capacity=3, num_pages=2, reward_material=0):
    ...
    self.reward_material = reward_material
    self.reward_items = []
  ...
  def render_state_str(self):
    passages_str = f"({self.pass_passages})" if self.pass_passages else ""
    if self.blockaded:
      passages_str = colored(passages_str, "red")
    heat_str = colored("!" * self.heat, "red") if self.heat else ""
    ambient_energy_str = f"{colored(energy_pip_symbol, energy_color_map[self.ambient_energy])}"
    safehouse_str = f"{colored('◭' * self.safehouse.level, 'green')}" if self.safehouse.level else ""
    reward_material_str = (f"{colored(f'{self.reward_material}⛁', 'yellow')}"
                           if self.reward_material > 1 else "⛁"*self.reward_material)
    key_str = "🔑" if self.reward_items else ""
    str_pieces = [ambient_energy_str, passages_str, heat_str, safehouse_str, key_str, reward_material_str]
    return "".join([s for s in str_pieces if s])

  def render_preview(self):
    if not self.seen:
      return colored("???", "cyan")
    name_str = colored(f"{self.name}", "magenta") if self.name else ""
    return (name_str + " " + self.render_state_str()
  ...
  def prompt_flavor(self, player, traverse_type: Literal["fight", "navigate"]):
    if not self.name:
      print(colored("Occupying Enemies: ", "red") + ", ".join([es.name for es in self.guardian_enemy_sets]))
  ...
  def render(self):
    ...
    self.position_str = f"({self.position[0]}-{self.position[1]})"
    render_str = f"-------- {name_str} {blockaded_str} {self.position} : {self.render_state_str()} --------\n"
...
class Region:
  def __init__(self, position, width, height, spell_pool, faction_set,
    ...
    for i in range(1, height + 1):
      ...
      for j in range(width):
        library = [LibrarySpell(sp, copies=1) for sp in self.spell_pool[spell_pool_cursor:spell_pool_cursor + 2]]
        ...
        node = Node(Safehouse(library), [guardian_enemy_wave], (i, j), seen=False)
    ...
    # Randomly seed some high reward material nodes, and the key node
    nodes = sum(self.nodes[1:-1], []) # exclude boss layer and starting layer
    chosen_nodes = random.sample(nodes, 5)
    # medium_reward_nodes = chosen_nodes[0:2]
    # high_reward_nodes = chosen_nodes[2:3]
    key_nodes = chosen_nodes[3:4]
    # for node in medium_reward_nodes:
    #   node.reward_material = 5
  ...
  def get_route_options(self, player) -> List[Tuple]:
    ...
    if current_layer_idx == len(self.nodes) - 2:
      ...
      print(f"---------------- CURRENT COLUMN = {player.current_column}")
    ...
    route_cost = player.find_route_cost
    while True:
      # render the routes
      ...
      for node, encounter in routes:
        print(node.render())
        print()
      still_exploring = choose_binary(colored(f"Find another route? (Costs {route_cost}hp, you're at {player.hp})", "red"))
      if still_exploring:
        ...
        player.hp -= route_cost
        route_cost += player.find_route_cost_scale_amount
      else:
        break
    return routes
  ...
  def render_layer(self, layer_idx):
    ...
    if self.enemy_view:
      ...
      get_node_content = lambda node, i: (node.safehouse.library[i].spell.description if node.seen else "???") if (i < len(node.safehouse.library) and node.safehouse.library[i].copies_remaining > 0) else "   "
...
class Map:
  ...
  def init(self, character=None) -> Player:
    ...
    character, starting_position = self.inspect()

    # Resolve the character
    if character is None:
      print("Starting a new character...")
      signature_spell_options = generate_library_spells(2, spell_pool=self.regions[0].spell_pool)
    ...
    if starting_position is None:
      starting_region = self.regions[0]
      col_raw = input("What column will you start in > ")
      starting_node = self.regions[0].nodes[0][int(col_raw)]
      player.current_column = int(col_raw)
    else:
      starting_region = self.regions[starting_position[0]]
      starting_node = starting_region.nodes[starting_position[1]][starting_position[2]]

    self.current_region_idx = starting_region.position
  ...
  def inspect(self) -> Tuple[Player, Optional[Tuple[int, int, int]]]:
    ...
    while (cmd := input("inspect the map > ")) != "done":
      ...
      try:
        ...
        elif cmd_tokens[0] == "bench":
          if len(cmd_tokens) == 1:
            print(numbered_list(self.inactive_characters.values()))
          else:
            character_idx = int(cmd_tokens[1]) - 1
            self.inactive_characters[character_idx].inspect()
        elif cmd_tokens[0] == "character":
          character_idx = int(cmd_tokens[1]) - 1
          last_inspected_node.safehouse.resting_characters[character_idx].inspect()
        elif cmd_tokens[0] == "start":
          character_name = cmd_tokens[1]
          character = self.inactive_characters.pop(character_name)
          return character, None
        elif cmd_tokens[0] == "continue":
          region = last_inspected_region
          layer_idx = int(cmd_tokens[1])
          column_idx = int(cmd_tokens[2])
          return None, (region.position, layer_idx, column_idx)
        elif cmd == "log":
