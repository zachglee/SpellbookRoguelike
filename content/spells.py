...
red_enemy_dies = [
  [Spell("When an enemy dies, gain 4 empower and deal overkill damage to immediate.", color="red", type="Passive",
         triggers_on=passive_for_death_overkill, raw_commands=["empower p 4", "damage i ^"]),
  Spell("inflict 1 vulnerable", color="red", type="Producer", raw_commands=["vulnerable _ 1"]),
  Spell("Deal 9 damage. If this kills, +1 red.", color="red", type="Converter", conversion_color="gold",
        targets=["_"], raw_commands=["damage _ 9"], generate_commands_post=if_kill("_", ["red p 1"])),
  Spell("Deal 16 damage", color="red", type="Consumer", raw_commands=["damage _ 16"])],
  #
  [Spell("When an enemy dies, inflict 4 burn on random.", color="red", type="Passive",
         triggers_on=passive_for_death_overkill, raw_commands=["burn r 4"]),
  Spell("inflict 2 burn.", color="red", type="Producer", raw_commands=["burn _ 2"]),
  Spell("Inflict 5 burn.", color="red", type="Converter", conversion_color="blue", raw_commands=["burn _ 5"]),
  Spell("Inflict 2 burn, then inflict their burn on all enemies.", color="red", type="Consumer",
        targets=["_"], raw_commands=["burn _ 2"], generate_commands_post=for_burn("_", ["burn a *"]))],
]

red_take_damage = [
  [Spell("When you lose hp, gain 5 empowered.", color="red", type="Passive",
         triggers_on=passive_lose_hp, raw_commands=["empower p 5"]),
  Spell("deal 2 damage. If this kills an enemy, recharge, refresh, +1 time.", color="red", type="Producer",
        raw_commands=["damage _ 2"], generate_commands_post=if_kill("_", ["time -1", "recharge last"])),
  Spell("Gain 1 sharp for every missing 4 health.", color="red", type="Converter", conversion_color="gold",
        generate_commands_pre=for_player_missing_hp(4, ["sharp p *"])),
  Spell("8 damage to all enemies on target side.", color="red", type="Consumer", raw_commands=["damage iside 8"])],
  #
  [Spell("When you lose hp, inflict 3 burn on all enemies.", color="red", type="Passive",
         triggers_on=passive_lose_hp, raw_commands=["burn a 3"]),
  Spell("gain 2 regen and 2 burn.", color="red", type="Producer", raw_commands=["regen p 2", "burn p 2"]),
  Spell("Gain 2 regen. Inflict 3 burn.", color="red", type="Converter", conversion_color="gold", raw_commands=["regen p 2", "burn _ 3"]),  # NOTE: Purple
  Spell("Inflict 3 burn. Tick all burn and gain life equal to damage dealt in this way.", color="red", type="Consumer", raw_commands=["burn _ 3"])], 
]

red_big_attack = [
  [Spell("Gain 1 red, 1 sharp, and 1 prolific for every 6 damage you survive each round.", color="red", type="Passive",
         triggers_on=passive_for_survive_6_damage_in_turn, raw_commands=["red p ^", "sharp p ^", "prolific p ^"]),
  Spell("If you're at or below half health, +1 red.", color="red", type="Producer",
...
red_hit_big_enemy = [
  [Spell("The 1st time you damage each enemy in a turn, inflict 5 burn if at least 10hp remains.", color="red", type="Passive",
         triggers_on=passive_first_damage_10hp_remains, raw_commands=["burn ^ 5"]),
  Spell("Deal 4 damage to immediate.", color="red", type="Producer", raw_commands=["damage i 4"]),
  Spell("Apply 1 poison. Apply extra 1 poison for every 4 hp the target has.", color="red", type="Converter", conversion_color="blue",
        targets=["_"], generate_commands_pre=for_enemy_remaining_hp("_", 5, ["poison _ 1", "poison _ *"])), # NOTE: Purple
  Spell("Target loses half its remaining health.", color="red", type="Consumer",
        targets=["_"], generate_commands_pre=for_enemy_remaining_hp("_", 2, ["suffer _ *"]))],
  #
  [Spell("The 1st time you damage each enemy in a turn, gain 3 regen if at least 10hp remains.", color="red", type="Passive",
         triggers_on=passive_first_damage_10hp_remains, raw_commands=["regen p 3"]),
  Spell("Deal 6 damage to a target at or above 10 health.", color="red", type="Producer",
  ...
  Spell("Deal 6 damage. Stun 1 the target if 10 or more hp remains.", color="red", type="Converter", conversion_color="blue",
        targets=["_"], raw_commands=["damage _ 6"], generate_commands_post=if_enemy_hp("_", 10, ["stun _ 1"], above=True)),
  Spell("Deal damage equal to target's missing health.", color="red", type="Consumer",
        targets=["_"], generate_commands_pre=for_enemy_missing_hp("_", 1, ["damage _ *"]))],
]

red_first_3_turns = [
  [Spell("At start of first 3 turns, gain 3 fleeting sharp", color="red", type="Passive",
          triggers_on=passive_first_3_turns, raw_commands=["sharp p 3", "delay 0 sharp p -3"]),
  Spell("call 1, +3 time", color="red", type="Producer", raw_commands=["call 1", "time -3"]),
  Spell("Deal 2 damage to random x times, where x is 8 - turn number", color="red", type="Converter", conversion_color="blue",
        generate_commands_pre=for_turn_number(["repeat * damage r 2"], lambda t: 8 - t)),
  Spell("Call 1 three times. Gain 6 sharp.", color="red", type="Consumer", raw_commands=["call 1", "call 1", "call 1", "sharp p 6"])],
  #
  [Spell("At start of first 3 turns, gain 1 inventive and 1 prolific.", color="red", type="Passive",
          triggers_on=passive_first_3_turns, raw_commands=["inventive p 1", "prolific p 1"]),
  Spell("If this has 0 or less charges, gain dig deep 2.", color="red", type="Producer",
        generate_commands_post=if_spell_charges(0, ["dig p 2"], above=False)),
  Spell("Call 2. Gain 15 empower.", color="red", type="Converter", conversion_color="gold", raw_commands=["call 2", "empower p 15"]),
  Spell("Regen 1. If this has 0 or less charges, deal 12 damage to random twice.", color="red", type="Consumer",
        raw_commands=["regen p 1"], generate_commands_post=if_spell_charges(0, ["damage r 12", "damage r 12"], above=False))]
]

red_random_target = [
  [Spell("At turn start, inflict 1 vulnerable on a random enemy", color="red", type="Passive",
          triggers_on=passive_turn_start, raw_commands=["vulnerable r 1"]),
  Spell("deal 3 damage to a random enemy.", color="red", type="Producer", raw_commands=["damage r 3"]),
  Spell("Gain 2 sharp this turn, inflict 2 vulnerable on all enemies.", color="red", type="Converter", conversion_color="gold",
...
blue_block_hits = [
  [Spell("When you’re attacked and take no damage, deal 6 damage to attacker and shield 1.", color="blue", type="Passive",
  ...
  #
  [Spell("When you’re attacked and take no damage, gain 3 retaliate and shield 1", color="blue", type="Passive",
...
blue_turn_3 = [
  ...
  Spell("ward 1.", color="blue", type="Producer", raw_commands=["ward p 1"]),
  Spell("At the end of next turn, all enemies take 8 damage.", color="blue", type="Converter", conversion_color="red",
        raw_commands=["delay 1 damage a 8"]),
  Spell("Ward 4.", color="blue", type="Consumer", raw_commands=["ward p 4"])],
]
...
blue_3_enemies = [
  ...
  Spell("Deal 6 damage to all stunned enemies. Stun 1 all enemies.", color="blue", type="Consumer", raw_commands=["stun a 1"])],
  #
  ...
  Spell("deal 6 damage, call 1", color="blue", type="Producer", raw_commands=["damage _ 6", "call 1"]),
  Spell("Gain retaliate 6 this turn.", color="blue", type="Converter", conversion_color="gold",
        raw_commands=["retaliate p 6", "delay 0 retaliate p -6"]),
  Spell("Gain 3 enduring this turn.", color="blue", type="Consumer",
        raw_commands=["enduring p 3", "delay 0 enduring p -3"])],
]

blue_excess_block = [
  [Spell("Excess block/shield at turn end is dealt as damage to immediate.", color="blue", type="Passive",
         triggers_on=passive_block_and_shield_at_end, raw_commands=["damage i ^"]),
  Spell("Gain 4 block.", color="blue", type="Producer", raw_commands=["block p 4"]),
  Spell("Gain 1 armor and 1 retaliate.", color="blue", type="Converter", conversion_color="red", raw_commands=["armor p 1", "retaliate p 1"]),
  Spell("Gain 20 block.", color="blue", type="Consumer", raw_commands=["block p 20"])],
  #
  [Spell("At turn end, gain shield equal to block.", color="blue", type="Passive",
         triggers_on=passive_block_at_end, raw_commands=["shield p ^"]),
  Spell("+2 shield.", color="blue", type="Producer", raw_commands=["shield p 2"]),
  Spell("Block 2. Convert block into 2x shield.", color="blue", type="Converter", conversion_color="gold",
        raw_commands=["block p 2"], generate_commands_post=for_player_condition(["block"], ["shield p *", "block p =0"], lambda b: 2*b)),
  Spell("Gain 7 block. Deal damage equal to block + shield to target.", color="blue", type="Consumer",
...
blue_no_enemy_deaths = [
  ...
  Spell("Encase 12 an enemy and inflict 2 doom.", color="blue", type="Consumer",
        raw_commands=["encase _ 12", "doom _ 2"])],
  #
  [Spell("At turn start, if no enemies are dead, ward self 1.", color="blue", type="Passive",
          triggers_on=passive_no_dead_enemies_at_begin, raw_commands=["ward p 1"]),
  Spell("Deal 6 damage to random 1 turn from now.", color="blue", type="Producer",
        raw_commands=["delay 1 damage r 6"]),
  Spell("Gain x time, x empower, and x block, where x is # of enemies", color="blue", type="Converter", conversion_color="gold",
        generate_commands_pre=for_enemies(["time -*", "empower p *", "block p *"])),
  Spell("Deal 2x damage to all enemies, where x is turn number.", color="blue", type="Consumer",
        generate_commands_pre=for_turn_number(["damage a *"], lambda s: 2*s))],
]

blue_on_entry = [
  [Spell("When an enemy enters, gain 1 retaliate and 2 block.", color="blue", type="Passive",
  ...
  Spell("Block 4. Break: Poison 4 all enemies.", color="blue", type="Converter", conversion_color="red",
  ...
  #
  [Spell("When an enemy enters, deal it 3 damage.", color="blue", type="Passive",
          triggers_on=passive_on_entry, raw_commands=["damage ^ 3"]),
  Spell("deal 5 damage to an enemy that entered this turn.", color="blue", type="Producer", raw_commands=["damage _entered 5"]),
  Spell("Deal 10 damage to the furthest faced enemy.", color="blue", type="Converter", conversion_color="gold",
        raw_commands=["damage distant 10"]),
  Spell("Banish 2 an enemy.", color="blue", type="Consumer", raw_commands=["banish _ 2"])]
]
...
gold_3rd_spell = [
  [Spell("When you cast your 3rd spell in a turn, gain 3 sharp.", color="gold", type="Passive",
  ...
  Spell("gain 1 regen.", color="gold", type="Producer", raw_commands=["regen p 1"]),
  Spell("+3 time. Every spell you cast after this gives 1 retaliation.", color="gold", type="Converter", conversion_color="blue", raw_commands=["time -3"]),
  Spell("Gain 5 searing presence.", color="gold", type="Consumer", raw_commands=["searing p 5"])],
]

gold_turn_page = [
  [Spell("When you turn to this page, recharge a random spell.", color="gold", type="Passive",
         triggers_on=passive_on_page, raw_commands=["recharge r"]),
  Spell("if this has 0 or less charges, gain 4 shield.", color="gold", type="Producer",
        generate_commands_post=if_spell_charges(0, ["shield p 4"], above=False)),
  Spell("Deal 6 damage to immediate. Gain 1 empower for each missing spell charge on this page.", color="gold", type="Converter", conversion_color="red",
        raw_commands=["damage i 6"], generate_commands_post=for_missing_charges(["empower p *"])),
  Spell("Gain Dig Deep 3. (Spells can go to -1 charge) Gain 3 shield.", color="gold", type="Consumer", raw_commands=["dig p 3", "shield p 3"])],
  #
...
gold_1_spell = [
  ...
  Spell("+1 time.", color="gold", type="Producer", raw_commands=["time -1"]),
  Spell("Deal 6 damage to immediate. If this kills, recharge and refresh.", color="gold", type="Converter", conversion_color="red",
        targets=["i"], raw_commands=["damage i 6"], generate_commands_post=if_kill("i", ["recharge last"])),
  Spell("Gain 3 prolific.", color="gold", type="Consumer", raw_commands=["prolific p 3"])],
]

gold_face_noone = [
  [Spell("At turn end, if facing no enemies, gain 2 searing presence.", color="gold", type="Passive",
         triggers_on=passive_face_noone_at_end, raw_commands=["searing p 2"]),
  Spell("gain 2 searing presence.", color="gold", type="Producer", raw_commands=["searing p 2"]),
  Spell("Banish immediate enemy.", color="gold", type="Converter", conversion_color="red",
        targets=["i"], raw_commands=["banish i"]),
  Spell("Gain 2 armor.", color="gold", type="Consumer", raw_commands=["armor p 2"])],
  #
  [Spell("At turn end, if facing no enemies, gain 3 shield.", color="gold", type="Passive",
         triggers_on=passive_face_noone_at_end, raw_commands=["shield p 3"]),
  Spell("if facing no enemies, empower 5.", color="gold", type="Producer",
        generate_commands_pre=if_facing_none(["empower p 5"])),
  Spell("Gain 5 shield.", color="gold", type="Converter", conversion_color="blue", raw_commands=["shield p 5"]),
  Spell("Deal 10 damage to immediate. If this kills an enemy, empower 10.", color="gold", type="Consumer",
        targets=["i"], raw_commands=["damage i 10"], generate_commands_post=if_kill("i", ["empower p 10"]))],
]

gold_first_face = [
  [Spell("The first time you face in a turn, gain 6 block.", color="gold", type="Passive",
          triggers_on=passive_first_face, raw_commands=["block p 6"]),
  Spell("Stun 1 immediate behind.", color="gold", type="Producer", raw_commands=["stun bi 1"]),
  Spell("Face, gain 2 shield for each enemy behind.", color="gold", type="Converter", conversion_color="blue",
        raw_commands=["face!"], generate_commands_post=for_enemies(["shield p *"], magnitude_func=lambda e: 2*e, specifier="behind")),
  Spell("Stun faced side 1, face, deal 8 damage to immediate.", color="gold", type="Consumer",
        raw_commands=["stun iside 1", "face!", "damage i 8"])],
  #
  [Spell("The first time you face in a turn, deal 6 damage to immediate.", color="gold", type="Passive",
          triggers_on=passive_first_face, raw_commands=["damage i 6"]),
  Spell("inflict 3 vulnerable on immediate behind.", color="gold", type="Producer", raw_commands=["vulnerable bi 3"]),
  Spell("Gain 2 sharp this turn, face, deal 4 to immediate.", color="gold", type="Converter", conversion_color="red",
        raw_commands=["sharp p 2", "delay 0 sharp p -2", "face!", "damage i 4"]),
  Spell("Deal 6 damage to immediate. If facing no enemies, face and deal 16 damage to immediate.", color="gold", type="Consumer",
        raw_commands=["damage i 8"], generate_commands_post=if_facing_none(["face!", "damage i 16"]))]
]

gold_use_last_charge = [
  [Spell("When you cast a spell's last charge, gain 4 shield.", color="gold", type="Passive",
         triggers_on=passive_use_last_charge, raw_commands=["shield p 4"]),
  Spell("gain 1 inventive", color="gold", type="Producer", raw_commands=["inventive p 1"]),
  Spell("Gain 1 evade, turn the page, +1 time.", color="gold", type="Converter", conversion_color="blue",
        raw_commands=["evade p 1", "page!", "time -1"]),
  Spell("Deal 5 to a random enemy x times, where x is amount of energy.", color="gold", type="Consumer",
        generate_commands_pre=for_player_energy(["repeat * damage r 5"]))],
  #
  [Spell("When you cast a spell's last charge, gain 3 searing presence.", color="gold", type="Passive",
          triggers_on=passive_use_last_charge, raw_commands=["searing p 3"]),
  Spell("If this has 0 or less charges, damage immediate 8.", color="gold", type="Producer",
